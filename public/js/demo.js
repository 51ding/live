...
function OnInit() {
    consoleLog("OnInit");
    sdk = new ILiveSDK(1400084330, 25442, "iLiveSDKCom");
...
function OnBtnReg() {
    ...
    ajaxPost(g_serverUrl + "/register", JSON.stringify(jsonObj),
        function(rspJson) {
            toastr.success("注册成功");
        }
    );
...
function OnBtnLogin() {
    ...
    ajaxPost(g_serverUrl + "/login", JSON.stringify(jsonObj),
        function(rspJson) {
...
function OnBtnCreateRoom(cb,rotate) {
    ...
    ajaxPost(g_serverUrl + "/createroom", JSON.stringify(jsonObj),
        function(rspJson) {
            sdk.createRoom(rspJson.data.roomnum, E_iLiveAuthBits.AuthBit_LiveMaster, "LiveMaster", function() {
                ...
                ajaxPost(g_serverUrl + "/reportroom", JSON.stringify(jsonObj),
                    function(rspJson) {
                        cb(g_roomnum);
                        report({
                            "token": g_token,
